...
resource "aws_security_group" "eks_nodes_sg" {
  name        = "${var.cluster.name}-eks-nodes-sg"
...
resource "aws_security_group_rule" "myip_to_worker_node_ingress_ssh" {
  type              = "ingress"
  from_port         = 22
  to_port           = 22
  protocol          = "tcp"
  ...
  description       = "Allow my IP to communicate with worker nodes on SSH"
}